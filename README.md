...
The worker foundation pre-creates a private Pi session with the assigned root's `TaskCreated` event before process launch, adopts that source on worker startup, and keeps semantic state out of a private atomic routing/liveness registry. Run-wide lease files enforce concurrency, routed status keeps registry-derived failure separate from worker-owned semantic status, and worker visibility is limited to the assigned subtree plus explicit context grants. The public asynchronous spawn/poll/join tools and task-aware global Pi compaction are added in the next milestones.
